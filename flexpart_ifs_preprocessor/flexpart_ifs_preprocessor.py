...
from flexpart_ifs_preprocessor.domain.data_model import  InputDataAggregatorEvent, Stream, Feed
...
def _parse_event_records(event: ConsumerRecords) -> list[InputDataAggregatorEvent]:
    ...
    for _, kafka_events in event['records'].items():
        for kafka_event in kafka_events:
            ...
            if event_value.stream in {Stream.S4Y, Stream.S5Y, Stream.S6Y} and event_value.domain in {Feed.F1, Feed.F2}:
